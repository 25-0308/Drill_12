...
class Zombie:
    ...
    def set_target_location(self, x=None, y=None):
        if not x or not y:
            raise ValueError('x, y 좌표를 모두 지정해야 합니다.')
        self.tx, self.ty = x, y
    ...
    def build_behavior_tree(self):
        ...
        root = move_to_target_location = Sequence('목적지로 이동하기',a1,a2)

        a3 = Action('랜덤 목적지 설정',self.set_random_location)
        root = wander = Sequence('배회하기',a3,a2)
        ...
        a5 = Action('순찰지점 설정',self.get_patrol_location)
        root = patrol = Sequence('순찰하기',a5, a2)

        root = chase_or_nearby = Selector('추적하거나 배회', chase_boy_if_nearby, patrol)
